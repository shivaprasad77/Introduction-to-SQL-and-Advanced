...
VALUES (1, 'Shifa', 'DA', 55000.00, '2023-06-15');
...
INSERT INTO employees VALUES (1, 'Shifa', 'Da');
-- This will fail:
-- INSERT INTO employees VALUES (1, 'Khusi', 'Finance');
...
-- Valid entry
INSERT INTO payroll VALUES (1, 'Shifa', 50000.00);
...
1️. LIMIT → Specifies how many records to return.
2️. OFFSET → Specifies how many records to skip before starting to return rows.
*/
...
------------------------------------------------------------
1️. Improves readability and organization of complex queries.  
2️. Allows reusing intermediate query results.  
3️. Helps avoid subquery repetition.  
4️. Enables recursive queries (like hierarchical data traversal).
============================================================
*/
...
-- Explanation:
-- 1️. The CTE `regional_avg` computes the average sale per region.
-- 2️. The main query compares each employee’s sales with their regional average.
-- 3️. Only employees whose sales exceed the average are displayed.
...
SELECT 
    ...
    Customer.Email,
    COUNT(Orders.OrderID) AS Total_Number_Of_Orders
FROM Customers customer
LEFT JOIN Orders Orders ON customer.CustomerID = Orders.CustomerID
GROUP BY Customer.CustomerID, Customer.CustomerName, Customer.Email
...
SELECT 
        ...
        Products.ProductName,
        Products.Price,
        ROW_NUMBER() OVER (PARTITION BY Categories.CategoryName ORDER BY Products.Price DESC) AS 'rank'
FROM 
        Products
INNER JOIN 
        Categories ON Products.CategoryID = Categories.CategoryID
...
#####################################################################################
##1. Top 5 customers by total amount spent (name, email, total spent):##
...
##2.Top 3 movie categories by rental counts (category name and rental count):##
...
##3.Number of films available per store and how many never rented:##
...
##6.Customers who rented more than 10 times in the last 6 months:##
